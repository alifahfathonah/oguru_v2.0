...
class Admin extends CI_Controller {
  ...
  public function categories($param1 = "", $param2 = "") {
    ...
    $page_data['categories_a'] = $this->crud_model->get_categories(2);
    $page_data['categories_v'] = $this->crud_model->get_categories(1);
    $page_data['categories_y'] = $this->crud_model->get_categories(3);
...
public function mitra($param1 = "", $param2 = "") {
    if ($this->session->userdata('admin_login') != true) {
      redirect(site_url('login'), 'refresh');
    }

    if ($param1 == "add") {
      $this->mitra_model->add_mitra();
      redirect(site_url('admin/mitra'), 'refresh');
    }
    elseif ($param1 == "edit") {
      $this->mitra_model->edit_mitra($param2);
      redirect(site_url('admin/mitra'), 'refresh');
    }
    elseif ($param1 == "delete") {
      $this->mitra_model->delete_mitra($param2);
      redirect(site_url('admin/mitra'), 'refresh');
    }elseif ($param1 == "aktif"){
      $this->mitra_model->aktif_mitra($param2);
      redirect(site_url('admin/mitra'), 'refresh');
    }elseif ($param1 == "nonaktif"){
      $this->mitra_model->nonaktif_mitra($param2);
      redirect(site_url('admin/mitra'), 'refresh');
    }


    $page_data['page_name'] = 'mitra';
    $page_data['page_title'] = 'Mitra';
    $page_data['mitra'] = $this->mitra_model->get_mitra();
    $this->load->view('backend/index', $page_data);
  }

  public function mitra_form($param1 = "", $param2 = "") {
    if ($this->session->userdata('admin_login') != true) {
      redirect(site_url('login'), 'refresh');
    }

    if ($param1 == 'add_mitra_form') {
      $page_data['page_name'] = 'mitra_add';
      $page_data['page_title'] = 'Tambah Mitra';
      $this->load->view('backend/index', $page_data);
    }
    elseif ($param1 == 'edit_mitra_form') {
      $page_data['page_name'] = 'mitra_edit';
      $page_data['mitra_id'] = $param2;
      $page_data['page_title'] = 'Edit Mitra';
      $this->load->view('backend/index', $page_data);
    }
  }
  ...
  public function konfirmasi_edukator($param1 = '', $param2 ='')
  {
    if ($this->session->userdata('admin_login') != 1)
    redirect(site_url('login'), 'refresh');
    if($param1 == 'detail' && $param2 != ''){
        $page_data['page_name'] = 'detail_pendaftar_edukator';
        $page_data['page_title'] = "Detail Pendaftar Edukator";
        $page_data['users'] = $this->user_model->get_edukator('detail', $param2)->row_array();
        if($page_data['users'] == ""){
          redirect(site_url('admin/konfirmasi_edukator'), 'refresh');
        }
        //die(print_r($page_data['users'])); 
        $this->load->view('backend/index', $page_data);
    }
    else if ($param1 == 'add') {
