...
class Crud_model extends CI_Model {
    ...
    public function get_kategori_home()
    {
        $this->db->select('category.*');
        $this->db->from('category');
        $this->db->join('course', 'category.id = course.category_id', 'left');
        $this->db->where('course.status', 'active');
        $query = $this->db->get();
        return $query->result_array();
    }
    public function get_my_courses_by_category_id_home($category_id) {
        $this->db->select('*');
        $this->db->from('course');
    ...
    public function purchase_history($user_id) {
        return $this->db->get_where('payment_mid', array('id_pengirim'=> $user_id, 'transaction_status' => 'settlement'));
    }

    public function get_payment_details_by_id($payment_id = "") {
        return $this->db->get_where('payment_mid', array('id' => $payment_id))->row_array();
    }
    ...
    public function get_instructor_wise_payment_history($instructor_id = "") {
        ...
        // $p = strtotime(date('D, d-M-Y'));
        // echo '<script type="text/javascript"> console.log("id = '.$p.'")</script>';
        if (sizeof($courses) > 0) {
            return $this->db->get_where('payment_mid', array('id_penerima' => $instructor_id))->result_array();
        }else {
            return array();
        }
    }
    ...
    public function update_frontend_settings() {
        ...
        $data['value'] = $this->input->post('informasi_edukator');
        $this->db->where('key', 'informasi_edukator');
        $this->db->update('frontend_settings', $data);
    }
    ...
    function send_new_private_message() {
        ...
        if ($num1 == 0 && $num2 == 0) {
            ...
            $data_notif['id_user'] = $receiver;
            $data_notif['tipe'] = 'message';
            $data_notif['id_target'] = $message_thread_code;
            $data_notif['date_add'] = $timestamp;
    ...
    function filter_vokasional($selected_category_id = "", $selected_price = "", $selected_rating = ""){
        //echo $selected_category_id.' '.$selected_price.' '.$selected_level.' '.$selected_language.' '.$selected_rating;
    ...
    function filter_akademik($selected_category_id = "", $selected_price = "", $selected_rating = ""){
        //echo $selected_category_id.' '.$selected_price.' '.$selected_level.' '.$selected_language.' '.$selected_rating;
        ...
        $this->db->where('parent_category', 2);
        $this->db->where('status', 'active');
        $courses = $this->db->get('course')->result_array();
    ...
    public function upload_pendaftaran_edukator(){
        
        // $data['id_edukator'] = $this->session->userdata('user_id');
        // $data['nama_file'] = md5(uniqid(rand(), true));
        // $data['status'] = 0;
        // $data['nik'] = $this->input->post('nik');
        // $data['nohp'] = $this->input->post('nohp');
        // $data['bidang'] = $this->input->post('bidang');
        // if($data['bidang'] == 'Vokasional'){
        //     $data['fokus_bidang'] = $this->input->post('fokus_vokasi');
        // }else{
        //     $data['fokus_bidang'] = $this->input->post('fokus_akademik');
        // }
        ...
        // // die(print_r($data));
        // move_uploaded_file($_FILES['file_ktp']['tmp_name'], 'uploads/edukator_file/foto_ktp/'.$data['nama_file'].'.jpg');
        // move_uploaded_file($_FILES['file_diriktp']['tmp_name'], 'uploads/edukator_file/foto_diri_ktp/'.$data['nama_file'].'.jpg');
        // move_uploaded_file($_FILES['file_cv']['tmp_name'], 'uploads/edukator_file/cv/'.$data['nama_file'].'.'.end(explode('.', $_FILES['file_cv']['name'])));
        // move_uploaded_file($_FILES['file_port']['tmp_name'], 'uploads/edukator_file/port/'.$data['nama_file'].'.'.end(explode('.', $_FILES['file_port']['name'])));
        // move_uploaded_file($_FILES['file_silabus']['tmp_name'], 'uploads/edukator_file/silabus/'.$data['nama_file'].'.'.end(explode('.', $_FILES['file_silabus']['name'])));
        // move_uploaded_file($_FILES['file_vid']['tmp_name'], 'uploads/edukator_file/contoh_video/'.$data['nama_file'].'.mp4');

        // $this->db->insert('pendaftaran_edukator', $data);
        // $datauser['is_edukator'] = 2;
        // $this->db->where('id', $this->session->userdata('user_id'))->update('users', $datauser);
        
        // die($_FILES['file_video']);
        // die($_FILES['file_ktp']['type']);
        $kesalahan = 0;
        if($_FILES['file_ktp']['type'] != "image/jpeg"){
            $kesalahan = 1;
            $this->session->set_flashdata('error_message', 'Foto KTP Harus dengan Format jpg !');
            redirect('home/pendaftaran_edukator', 'refresh');
        }

        if($_FILES['file_diriktp']['type'] != "image/jpeg"){
            $kesalahan = 1;
            $this->session->set_flashdata('error_message', 'Foto diri dengan KTP Harus dengan Format jpg !');
            redirect('home/pendaftaran_edukator', 'refresh');
        }

        if($_FILES['file_cv']['type'] != "application/pdf"){
            $kesalahan = 1;
            $this->session->set_flashdata('error_message', 'File Curiculum Vitae Harus dengan Format jpg !');
            redirect('home/pendaftaran_edukator', 'refresh');
        }

        if($_FILES['file_port']['type'] != "application/pdf"){
            
            $kesalahan = 1;
            $this->session->set_flashdata('error_message', 'File Protofolio Harus dengan Format pdf !');
            redirect('home/pendaftaran_edukator', 'refresh');
        }
        if($_FILES['file_silabus']['type'] != "application/pdf"){
            $kesalahan = 1;
            $this->session->set_flashdata('error_message', 'File Silabus Harus dengan Format pdf !');
            redirect('home/pendaftaran_edukator', 'refresh');
        }
        
        // die(print_r($_FILES['file_vid']['size']));
        
        if($_FILES['file_vid']['name']){
            if($_FILES['file_vid']['type'] != "video/mp4"){
                $kesalahan = 1;
                $this->session->set_flashdata('error_message', 'File Video Harus dengan Format mp4 !');
                redirect('home/pendaftaran_edukator', 'refresh');
            }
            if($_FILES['file_vid']['size'] <= 104857600 ){
                
            }else{
                $kesalahan = 1;
                $this->session->set_flashdata('error_message', 'File Contoh Video Harus Kurang Dari 100');
                redirect('home/pendaftaran_edukator', 'refresh');
            }
        }
        ...
        if($kesalahan == 0){
            ...
            $this->db->insert('pendaftaran_edukator', $data);
            $datauser['is_edukator'] = 2;
            $datauser['status_edukator'] = $this->input->post('tipe_edukator');
            $this->db->where('id', $this->session->userdata('user_id'))->update('users', $datauser);
        }

        // move_uploaded_file($_FILES['file_ktp']['tmp_name'], 'uploads/edukator_file/'.);
    }
    ...
    public function add_notif_edukator()
    {
        ...
        $this->email_model->send_mail_edukator_aktif($user_details['email'],$user_details['id'],$data['pesan']);
        $data_user['is_edukator'] = $status;
        $data_pendaftaran['status'] = 1;
        $this->db->where('id_edukator', $data['id_user'])->where('status', 0)->update('pendaftaran_edukator', $data_pendaftaran);
