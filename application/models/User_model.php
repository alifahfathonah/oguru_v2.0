...
class User_model extends CI_Model {
    ...
    public function get_edukator($param1 = '', $param2 = '')
    {
        if ($param1 == 'konfirmasi') {
            $this->db->where('is_edukator', 2);
            $this->db->where('role_id', 2);
            return $this->db->get('users');

        }else if($param1 == 'detail' && $param2 != ''){
            $this->db->select('users.id, users.is_edukator, users.role_id, users.email, users.first_name ,users.last_name, users.status_edukator, pendaftaran_edukator.*');
            $this->db->from('users');
            $this->db->join('pendaftaran_edukator', 'users.id = pendaftaran_edukator.id_edukator', 'left');
            $this->db->where('users.is_edukator', 2);
            $this->db->where('users.role_id', 2);
            $this->db->where('users.id', $param2);
            $this->db->where('pendaftaran_edukator.status', 0);
            return $this->db->get();
