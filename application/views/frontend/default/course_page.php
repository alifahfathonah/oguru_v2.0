    ...
    <div class="row align-items-end">
      <div class="col-lg-8">
        <div class="course-header-wrap">
          <h1 class="title"><?php echo $course_details['title'].' ('.$course_details['tipe'].')'; ?></h1>
        ...
        <div class="created-row">
          <span class="created-by">
            Dibuat oleh 
            <a class="mr-4" href="<?php echo site_url('home/edukator_info/'.$course_details['user_id']); ?>"><b><?php echo $instructor_details['first_name'].' '.$instructor_details['last_name']; ?> </b></a>
            Status Edukator
            <a href="<?php echo site_url('home/informasi_edukator').'#On'.$instructor_details['status_edukator']; ?>" target="_blank" class="mr-1"><b><?php echo $instructor_details['status_edukator']; ?></b></a>
            <a href="<?php echo site_url('home/informasi_edukator').'#On'.$instructor_details['status_edukator']; ?>" target="_blank" class="fas fa-question-circle"></a>
          </span>
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-8">
        ...
        <?php if($course_details['tipe'] == 'online'){ ?>
          <br>
    ...
    <?php if($course_details['tipe'] == 'webinar'){ ?>
      <div class="description-box view-more-parent">
        <div class="description-title" style="font-size: 18px; font-weight: 550;"><i class="fas fa-calendar-alt"></i> Jadwal Pelaksanaan</div>
        <div class="description-content-wrap">
          <div class="description-content" style="margin-left: 25px;">
            <?php 
              $daftar_hari = array(
                'Sunday' => 'Minggu',
                'Monday' => 'Senin',
              ...
              $tgl = date('d-m-Y', strtotime($course_details['tanggal']));
              $h_tgl = date('l', strtotime($course_details['tanggal']));
              $hari = $daftar_hari[$h_tgl];
              echo $hari.', '.$tgl; 
              ?>
          </div>
          <div class="description-content" style="margin-left: 25px;">
            <?php 
              $wkt = explode(':', $course_details['waktu']);
              echo $course_details['waktu'].' - '.((int)$wkt[0]+$course_details['durasi']).':'.$wkt[1]; 
            ?> WIB
          </div>
        </div>
      </div>
      <div class="description-box view-more-parent">
        <div class="description-title" style="font-size: 18px; font-weight: 550;"><i class="fas fa-play"></i> Situs Webinar</div>
        <div class="description-content-wrap">
          <div class="description-content" style="margin-left: 25px;">
            <i class="fab fa-whatsapp"> <?php echo $course_details['grupchat'].' grup'; ?></i>
          </div>
          <div class="description-content" style="margin-left: 25px;">
            <i class="fas fa-video">  Via <?php echo $course_details['live']; ?></i>
          </div>
        </div>
      </div>
      <div class="description-box view-more-parent">
        <div class="description-title" style="font-size: 18px; font-weight: 550;"><i class="fas fa-users"></i> Kuota Kelas</div>
        <div class="description-content-wrap">
          <div class="description-content" style="margin-left: 25px;">
            <?php echo $course_details['kuota']; ?> Kursi
          </div>
          <div class="description-content" style="margin-left: 25px;">
            <?php 
              $jum_enrol = $this->db->get_where('enrol', array('course_id' => $course_details['id']))->num_rows();
              $jum_enrol = $course_details['kuota'] - $jum_enrol;
              echo "<b>".$jum_enrol.' Kursi Tersisa</b>';
             ?>
          </div>
        </div>
      </div>
    <?php }elseif($course_details['tipe'] == 'workshop'){ ?>
      <div class="description-box view-more-parent">
        <div class="description-title" style="font-size: 18px; font-weight: 550;"><i class="fas fa-calendar-alt"></i> Jadwal Pelaksanaan</div>
        <div class="description-content-wrap">
          <div class="description-content">
            <ul>
              <li>
                <?php echo '<b>'.date_format(date_create($course_details['tanggal']),"j F Y").' - '.date_format(date_create($course_details['tanggal_selesai']),"j F Y").'</b>'; ?>
              </li>
              <li>
                <?php echo '<b>'.$course_details['waktu'].' - selesai </b>'; ?>
              </li>
            </ul>
          </div>
        </div>
    ...
    <!-- <div class="compare-box view-more-parent">
      ...
      <div class="compare-courses-wrap">
        <?php
        $other_realted_courses = $this->crud_model->get_courses($course_details['category_id'])->result_array();
  ...
  <div class="course-sidebar natural">
    ...
    <div class="course-sidebar-text-box">
      ...
      <?php else: ?>
        <?php if ($course_details['is_free_course'] == 1): ?>
          <div class="buy-btns">
            <?php if ($this->session->userdata('user_login') != 1): ?>
              <a href = "#login" data-target="#login" data-toggle="modal" class="btn btn-buy-now" onclick="handleEnrolledButton()"><?php echo get_phrase('get_enrolled'); ?></a>
            <?php else: ?>
        ...
        <?php else: ?>
          <div class="buy-btns">
            <a href = "<?php $id = $course_details['id'];  echo site_url('home/daftar_kelas/'.$id); ?>" class="btn btn-buy-now" id = "course_<?php echo $course_details['id']; ?>"><?php echo get_phrase('buy_now'); ?></a>
          </div>
        <?php endif; ?>
      <?php endif; ?>
      ...
      <div class="includes">
        ...
        <ul>
            <li><i class="far fa-user"></i><?php echo $instructor_details['status_edukator'].' : '.$instructor_details['first_name'].' '.$instructor_details['last_name']; ?>
          </li>
          </li>
          <?php if($course_details['tipe'] == 'online'){ ?>
        ...
        <?php
                    ...
                    $dateMulai = date('Y-m-d', strtotime($course_details['tanggal']));
                    if($dateNow <= $dateMulai):
                 ?>
                 <i class="fa fa-calendar text-dark mr-1" ></i>
                 <small><span class="text-dark"><?= date_format(date_create($course_details['tanggal']),"j F Y");?></span></small>
                 <?php endif;?>
      </div>
    </div>
  </div>
